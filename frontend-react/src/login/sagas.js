...
const loginUrl = `${process.env.REACT_APP_API_URL}/api/v1/auth`

function loginApi (email, password) {
  return fetch(`${loginUrl}?user=${email}&password=${password}`, {
    method: 'POST',
    headers: {
...
function* loginWatcher () {
  ...
  while (true) {
    ...
    // ... finally we'll just log them out.  This will unset the user
